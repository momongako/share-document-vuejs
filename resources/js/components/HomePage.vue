    ...
    <div class="row content">
        <div style="margin-top: -20px" class="col-sm-2 col-md-2 col-lg-2 col-xl-2 sidenav sidenav bg-white ">
            <div class="show-list-document">
                <h4 class="document">
                    <router-link :class="[{ active: $route.name === 'index' }]" :to="{ name: 'index' }">
                        Document
                    </router-link>
                </h4>
            ...
            <div style="max-height: 300px;overflow-y: auto">
                <ul class="nav flex-column" v-show='!toggle_document'>
                    ...
                    <li class="nav-item" v-for="(category, index) in categories" :key="(index + 1)">
                        <router-link class="text-truncate" style="max-width: 190px;"
                            :to="{ name: 'getListPost', params: { id: category.id } }"
                            :class="[{ active: $route.name === 'getListPost' && $route.params.id === category.id }]">
                            <!--table category-->
                            <div class="nav-item category-folder-show text-truncate" style="max-width: 190px;"
                                id="category-edit">
            ...
            <div style="max-height: 300px;overflow-y: auto">
                <ul class="nav flex-column list-category">
                    <li style="font-weight: 600;" v-show='!toggle' class="nav-item category-folder-show">
                        <router-link :class="[{ active: $route.name === 'addCategory' }]" :to="{ name: 'addCategory' }">
                            <i class="bi bi-folder-plus fa-lg"></i> &nbsp; New Category
                        </router-link>

                    </li>
                    <li v-for="(category, index) in categories" :key="(index + 1)" v-show='!toggle'
